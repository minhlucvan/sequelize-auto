...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      var attrStartIndex = text[table].length;
      ...
      _.each(fields, function(field, i){
        ...
        var attrEndIndex = text[table].length;
        var attrText = text[table].substr(attrStartIndex, attrEndIndex - attrStartIndex);
        migration[table] = self.renderMigration(table, attrText);
      ...
      if (self.options.directory) {
        return self.write(text, migration, typescriptFiles, callback);
      }
...
AutoSequelize.prototype.renderMigration  = function(tableName, attributesDef) {
  return `"use strict";
    var sequelize = require('sequelize');
    ...
    module.exports = {
      ...
      down: function(migration, DataTypes, done) {
        migration.dropTable("${tableName}").done(done);
      }
    };`
}

AutoSequelize.prototype.getCurrentYYYYMMDDHHmms = function() {
  const date = new Date();
  return [
    date.getUTCFullYear(),
...
AutoSequelize.prototype.write = function(attributes, migration, typescriptFiles, callback) {
  ...
  function createFile(table, _callback) {
    ...
    var filenameMigration = self.getCurrentYYYYMMDDHHmms() + '-create-' + fileName;
    fs.writeFile(path.resolve(path.join(self.options.directory, fileName + (self.options.typescript ? '.ts' : '.js'))), attributes[table]);
    fs.writeFile(path.resolve(path.join('migrations', filenameMigration + (self.options.typescript ? '.ts' : '.js'))), migration[table], _callback);
  }
};
